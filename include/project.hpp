...
public:
    ...
    const std::vector<std::filesystem::path>& getSubFolders() const { return subFolders; }

    const ProjectTemplate& getTemplate() const { return tmpl; }
    const std::filesystem::path& getProjectSubFolder() const { return projectSubFolder; }
    ...
    void setSubFolders(const std::vector<std::filesystem::path> s) { subFolders = s; }

    void setTemplate(const ProjectTemplate& t) { tmpl = t; }
    void setProjectSubFolder(const std::filesystem::path& p) { projectSubFolder = p; }
