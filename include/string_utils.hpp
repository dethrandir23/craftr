...
enum class Placeholder { ProjectName, AuthorName, Date };

inline std::string ToString(Placeholder ph) {
  switch (ph) {
  case Placeholder::ProjectName:
    return "{{PROJECT_NAME}}";
  case Placeholder::AuthorName:
    return "{{AUTHOR_NAME}}";
...
enum class ContentType { CMake, Readme, Metadata, Main, License };

inline std::string ToString(ContentType ct) {
  switch (ct) {
  case ContentType::CMake:
    return "cmake";
  case ContentType::Readme:
    return "readme";
