...
public:
  ...
  const std::vector<FileBlueprint> &GetFileBluePrints() const {
    return this->file_blueprints;
  }
  const std::map<std::string, std::string> &GetLicenses() const {
    return this->licenses;
  }

  void SetTemplatePath(const std::filesystem::path &path) {
    template_path = path;
  }
  ...
  void AddReplacer(const Replacer &r) { replacers.emplace_back(r); }

  void ClearReplacers() { replacers.clear(); }
  ...
  void ClearSubFolders() { subfolders.clear(); }
  ...
  void ClearLicenses() { licenses.clear(); }
