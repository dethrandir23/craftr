...
bool ProjectTemplate::LoadTemplate() {
    try {
        ...
        if (config["version"])     version = config["version"].as<std::string>();
        if (config["author"])      author = config["author"].as<std::string>();
        if (config["description"]) description = config["description"].as<std::string>();

        if (config["replacers"]) {
            for (auto it : config["replacers"]) {
                replacers.emplace_back(it.first.as<std::string>(), it.second.as<std::string>());
            }
        }
